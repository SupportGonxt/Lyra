...
const HAND_WRITTEN: Op[] = [
  ...
  { method: "put", path: "/v1/signal/creatives/{id}/design", summary: "Save a creative's studio layout and canvas edits (DesignJson). Not a publish: no approval, audited", permission: "signal:creatives:generate", tag: "signal", requestBody: true },
  { method: "post", path: "/v1/scout/signals/import", summary: "Import SCOUT signals from CSV (source, sourceRef, observedAt YYYY-MM-DD, weight; other columns become the payload). Stored by the harvest path, per-line honest", permission: "scout:signals:ingest", tag: "scout", requestBody: true },
  { method: "post", path: "/v1/core/customers/import", summary: "Bulk-import customers from CSV (name required; email, phone, locale, type, tags). Per-line honest; a known email merges tags. Consent is never imported", permission: "core:customers:create", tag: "core", requestBody: true },
